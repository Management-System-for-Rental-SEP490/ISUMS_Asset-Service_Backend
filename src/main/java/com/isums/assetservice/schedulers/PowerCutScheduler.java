...
public class PowerCutScheduler {
    ...
    public void executePendingPowerCuts() {
        ...
        List<PowerCutJob> jobs = powerCutJobRepo.findByStatusAndExecuteAtBefore(PowerCutJobStatus.PENDING, now);

        for (PowerCutJob job : jobs) {
            try {
                iotProvisioningService.sendPowerCutCommand(job.getHouseId());

                controllerRepository.findByHouseId(job.getHouseId()).ifPresent(ctrl -> {
                    ctrl.setPaymentCutActive(true);
                    ctrl.setActivePaymentCutJobId(job.getId());
                    controllerRepository.save(ctrl);
                    log.info("[PowerCut] paymentCutActive=true houseId={} jobId={}",
                            job.getHouseId(), job.getId());
                });

                int updated = areaPowerStateRepository.markAllAreasPaymentDue(job.getHouseId(), job.getId());
                if (updated > 0) {
                    log.info("[PowerCut] Marked {} areas as PAYMENT_DUE houseId={}", updated, job.getHouseId());
                }
                ...
                log.error("[PowerCut] Failed houseId={}: {}", job.getHouseId(), e.getMessage(), e);
            }
        }
    }
}
